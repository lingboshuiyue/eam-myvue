<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-date-picker v-model="listQuery.lendTime" type="datetime" clearable class="filter-item" placeholder="入库日期" @change="handleFilter" />
      ...
      <el-select v-model="listQuery.lendReturnType" placeholder="资产状态" clearable class="filter-item" style="width: 130px" @change="handleFilter">
        <el-option v-for="item in lendReturnTypes" :key="item.key" :label="item.label" :value="item.key" />
      </el-select>
    ...
    >
      ...
      <el-table-column label="来源" width="110px" align="center">
        <template v-if="scope.row.assets != null" slot-scope="scope">
          <span>{{ scope.row.assets.assetsSource }}</span>
        </template>
      </el-table-column>
      <el-table-column label="出入库日期" width="150px" align="center">
        <template v-if="scope.row.lendTime != null" slot-scope="scope">
          <span>{{ scope.row.lendTime | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="价值">
        <template v-if="scope.row.assets != null" slot-scope="scope">
          <span>{{ scope.row.assets.assetsValue }}</span>
        </template>
      </el-table-column>
      <el-table-column label="数量" align="center" width="95">
        <template v-if="scope.row.assets != null" slot-scope="scope">
          <span v-if="scope.row.assets.count">{{ scope.row.assets.count }}</span>
          <span v-else>0</span>
        </template>
      </el-table-column>
      <el-table-column label="预计归还日期" width="150px" align="center">
        <template v-if="scope.row.expectReturnTime != null" slot-scope="scope">
          <span>{{ scope.row.expectReturnTime | parseTime('{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="实际归还日期" width="150px" align="center">
        <template v-if="scope.row.realReturnTime != null" slot-scope="scope">
          <span v-if="scope.row.realReturnTime != null">{{ scope.row.realReturnTime | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="资产状态" class-name="status-col" width="100">
        <template v-if="scope.row.assets != null" slot-scope="scope">
          <el-tag v-if="scope.row.lendReturnStatus === false" type="danger">
            审核中
          </el-tag>
          <el-tag v-if="scope.row.repairStatus === true" type="danger">
            维修中
          </el-tag>
          <el-tag v-if="scope.row.repairStatus !== true && scope.row.lendReturnStatus !== false" :type="assetsStatusColorList[scope.row.lendReturnType-1]">
            {{ lendReturnTypeOptions[scope.row.lendReturnType-1] }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="320" class-name="small-padding fixed-width">
        <template v-if="row.assets != null && row.repairStatus !== true && row.lendReturnStatus !== false && row.lendReturnType !== 2" slot-scope="{row}">
          <el-button type="primary" size="mini" @click="handleModifyStatus(row, '2')">
            归还
          </el-button>
          <el-button type="danger" size="mini" @click="handleModifyStatus(row, '3')">
            报修
          </el-button>
        </template>
      </el-table-column>
...
import { fetchLendReturnList, fetchLendReturnAll } from '@/api/lend_return'
...
export default {
  ...
  data() {
    return {
      ...
      lendReturnTypes: [{ label: '正常', key: 1 }, { label: '归还', key: 2 }, { label: '维修', key: 3 }, { label: '审核', key: 4 }],
      statusOptions: ['published', 'draft', 'deleted'],
      // assetsStatusList: ['正常', '已借出', '维修', '报废', '变卖'],
      assetsStatusList: ['归还', '维修'],
      lendReturnTypeOptions: ['正常', '已归还', '正常'],

      assetsStatusColorList: ['success', 'danger', 'success'],
      showReviewer: false,
      temp: {
        id: undefined,
        importance: 1,
  ...
  methods: {
    getList() {
      ...
      this.listQuery.eid = this.personal_info.eid
      if (this.listQuery.lendTime != null) { this.listQuery.lendTimeTransfer = parseTime(this.listQuery.lendTime, '{y}{m}{d}') } else { this.listQuery.lendTimeTransfer = null }
      fetchLendReturnList(this.listQuery).then(response => {
      ...
      fetchTypesList().then(response => {
        ...
        console.log('测试返回的类型')
        console.log(this.assetsTypes)
        // // Just to simulate the time of the request
      })
      // fetchdepreciationWaysList().then(response => {
      //   this.depreciationWays = response.data
      ...
      this.$confirm('确定要申请' + hint + '？', '提示', {
        ...
        createApply(tempData).then(() => {
          for (const v of this.list) {
            if (v.lendId === this.apply.lendId) {
              const index = this.list.indexOf(v)
              v.lendReturnStatus = false
    ...
    handleDownload() {
      ...
      fetchLendReturnAll().then(response => {
        if (response.data != null) {
          const tempData = response.data
          console.log('测试')
          console.log(tempData)
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = ['编号', '资产编号', '名称', '来源', '出入库日期',
              '类型', '价值', '预计归还日期', '实际归还日期', '资产状态']
            const filterVal = ['lendId', 'assetsId', 'assetsName', 'assetsSource', 'lendTime',
              'assetsType', 'assetsValue', 'expectReturnTime', 'realReturnTime', 'lendReturnType']
            const data = this.formatJson(filterVal, tempData)

            excel.export_json_to_excel({
              header: tHeader,
              data,
              filename: 'lend_return-list_' + parseTime(new Date())
            })
    ...
    formatJson(filterVal, jsonData) {
      return jsonData.map(v => filterVal.map(j => {
        if (j === 'lendTime' || j === 'expectReturnTime' || j === 'realReturnTime') {
          if (v[j] != null) { return parseTime(v[j]) } else { return '' }
        } else if (j === 'assetsName') {
          return v['assets'].assetsName
        } else if (j === 'assetsType') {
          return v['assets'].type.typeName
        } else if (j === 'assetsValue') {
          return v['assets'].assetsValue
        } else if (j === 'lendReturnType') {
          if (v['lendReturnStatus'] === false) { return '审核中' } else if (v['repairStatus'] === true) { return '维修中' } else { return lendReturnTypeOptions[v[j] - 1] }
        } else {
          return v[j]
        }
      }))
